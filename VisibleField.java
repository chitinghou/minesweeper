...
public class VisibleField {
   ...
   public VisibleField(MineField mineField) {
      ...
      guessedMine = this.mineField.numMines();
      this.row = this.mineField.numRows();
      this.col = this.mineField.numCols();
      this.gridNum = this.row * this.col;
      
      
      this.status = new int[this.row][this.col];
   ...
   */     
   ...
   public void cycleGuess(int row, int col) {
      if(this.status[row][col] == COVERED){
         ...
         this.guessedMine++;
      }else if(this.status[row][col] == QUESTION){
   ...
   public boolean uncover(int row, int col) {
      if(this.mineField.hasMine(row, col)){
         this.status[row][col] = EXPLODED_MINE;
         return false;
      }else{
   ...
   public boolean isGameOver() {
      for(int i=0; i<this.row; i++){
         for(int j=0; j<this.col; j++){
            if(this.status[i][j] == INCORRECT_GUESS || this.status[i][j] == EXPLODED_MINE || this.status[i][j] == MINE){
               lose();
               return true;
            }
         }
      }
      if(this.gridNum - this.uncoveredNum == this.mineField.numMines()){
         win();
         return true;
      }
      return false;      
   ...
   //when the user win, change the mineField 's status to show the result.
   private void win(){
      for(int i=0; i<this.row; i++){
         for(int j=0; j<this.col; j++){
            if(this.status[i][j] == COVERED){
               this.status[i][j] = MINE_GUESS;
            }
         }
      }
   }
   //when the user lose, change the mineField 's status to show the result.
   private void lose(){
      for(int i=0; i<this.row; i++){
         for(int j=0; j<this.col; j++){
            if(this.status[i][j] == MINE_GUESS && !this.mineField.hasMine(i, j)){
               this.status[i][j] = INCORRECT_GUESS;
            }
            if(this.mineField.hasMine(i, j)){
               if(this.status[i][j] == COVERED){
                  this.status[i][j] = MINE;
               }
            }
         }
      }
   ...
   //when the user reset, change the mineField 's status to COVERED.
